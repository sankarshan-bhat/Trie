...
private:
	...
	void displayTrie(Node<T>*, T*, int i, const int length) ;
...
*/
		...
		void operator++()
		{
			...
			if ( !st.empty() ) 
			{
				...
				//cout<<"in stack pop is "<<pop<<endl;
				//cout<<"in list  is "<<last<<endl;
				l_.pop_back();
				st.pop();
				pop=pop->rptr;
				while(!(pop->isWord))
...
template <typename T>
void Trie<T>::displayTrie(Node<T> *ptrNode, T str[], int i, const int length)
{
	if (ptrNode) {
		if (ptrNode->isWord && i <= length) {
			str[i] = '\0';
			std::cout << str << "\n";
		}
		if (ptrNode->lptr) {
		...
		displayTrie(ptrNode->lptr, str, i + 1, length);
		
		if (ptrNode->rptr) {
			str[i - 1] = ptrNode->rptr->data;
			
			displayTrie(ptrNode->rptr, str, i, length);
		}
...
void Trie<T>::display() 
{
	...
	if (ptrNode->lptr) 
	{
		...
		T cstr[n+1];
	        cstr[0] = { ptrNode->data };
		displayTrie(ptrNode, cstr, 1, n+1);
	}
}
